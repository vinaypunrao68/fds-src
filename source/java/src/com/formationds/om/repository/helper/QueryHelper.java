    ...
    public Statistics execute( final MetricQueryCriteria query, final Authorizer authorizer,
        ...
        if ( query != null ) {
            ...
            try {
                ...
                final List<IVolumeDatapoint> queryResults = (List<IVolumeDatapoint>)
                    SingletonRepositoryManager.instance().getMetricsRepository().query( query );
...
//		                calculatedList.add( percentageFull( consumed, systemCapacity ) );
	            	...
	            	try {
		            	...
		            	gets.getDatapoints().forEach(
		            		gPoint -> {
		            			ssdGets.getDatapoints().stream().filter( sPoint -> sPoint.getX()
                                                                                         .equals( gPoint.getX() ) )
		            				.forEach(
                                                sPoint -> {
