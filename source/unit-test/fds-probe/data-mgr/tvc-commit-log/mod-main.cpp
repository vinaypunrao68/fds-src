...
class CommitLogProbeTest : public FdsProcess {
  public:
    CommitLogProbeTest(int argc, char **argv, Module **mod_vec)
            : FdsProcess(argc,
                         argv,
                         "platform.conf",
                         "fds.dm.",
                         "commit-log-probe.log",
                         mod_vec) {}
    virtual ~CommitLogProbeTest() {}

    void proc_pre_startup() override
    {
        // Add your probe adapter(s) to S3 connector.
        gl_probeS3Eng.probe_add_adapter(&gl_CommitLogProbe);
        for (int i = 0; i < 0; i++) {
            gl_probeS3Eng.probe_add_adapter(gl_CommitLogProbe.pr_new_instance());
        }
    }
...
int main(int argc, char **argv)
{
    fds::Module *probe_vec[] = {
        &fds::gl_fds_stat,
        &fds::gl_probeS3Eng,
        &fds::gl_DmCommitLogMod,
        &fds::gl_CommitLogProbe,
        NULL
    };
    fds::CommitLogProbeTest commitLogTest(argc, argv, probe_vec);
    return commitLogTest.main();
}
