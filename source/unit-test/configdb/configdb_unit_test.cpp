...
TEST_CASE("simple") {
    ...
    REQUIRE (cfg.addLocalDomain("local",0,0));
    REQUIRE (cfg.addLocalDomain("local2",1,0));

    std::map<int,std::string> mapDomains;
    
    cfg.getLocalDomains(mapDomains);

    REQUIRE (mapDomains.size() == 2);
    REQUIRE (mapDomains[0] == "local");
    REQUIRE (mapDomains[1] == "local2");
}
