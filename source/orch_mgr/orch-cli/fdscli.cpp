...
FdsCli::FdsCli(const boost::shared_ptr<FdsConfig>& omconf) :
    ...
    cli_log->setSeverityFilter((fds_log::severity_level) om_config->get<int>("fds.om.log_severity"));
        ...
        */
        ...
        if (vm.count("volume-policy")) {
            ...
            if (vm.count("iops-min") || vm.count("iops-max") || vm.count("rel-prio")) {
                FDS_PLOG_SEV(cli_log, fds::fds_log::notification) 
                        << "Since prolicy id is specified, min/max iops and priority will be ignored";
            }
        }
        else {
            if ((vm.count("iops-min")==0) || (vm.count("iops-max")==0) || (vm.count("rel-prio")==0)) {
                FDS_PLOG_SEV(cli_log, fds::fds_log::error)
                        << "If 'volume-policy' is not specified, must specify min/max iops and relative priority";
                return 0;
            }
        ...
        volData.vol_desc.defConsisProtocol = FDSP_ConsisProtoType::FDSP_CONS_PROTO_STRONG;
        volData.vol_desc.archivePolicyId = 0;
        volData.vol_desc.placementPolicy = 0;
        volData.vol_desc.appWorkload = FDSP_AppWorkload::FDSP_APP_WKLD_TRANSACTION;
        ...
        FDSP_CreatePolicyType policyData;
        ...
        FDSP_DeletePolicyType policyData;
        ...
        FDSP_ModifyPolicyType policyData;
        ...
        FDSP_CreateDomainType domainData;
        ...
        FDSP_GetDomainStatsType domainData;
        domainData.domain_id = vm["domain-id"].as<int>();
        ...
        FDSP_ThrottleMsgType throttle_msg;
