...
ObjectStorMgr::writeObjectToTier(const ObjectID  &objId, 
    ...
    err = writeObjectMetaData(objId, objData.data.length(), disk_req->req_get_phy_loc(), false, diskTier, &vio);
    if ((err == ERR_OK) &&
            (tier == diskio::flashTier)) {
        /*
...
ObjectStorMgr::relocateObject(const ObjectID &objId,
    ...
    err = writeObjectMetaData(objId, objGetData.data.length(), disk_req->req_get_phy_loc(), true, from_tier, &vio);
