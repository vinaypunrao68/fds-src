...
class TestBlockFioSeqW(TestCase.FDSTestCase):
    ...
    def test_BlockFioWrite(self):
        ...
        fioCmd = "sudo fio --name=seq-writers --readwrite=write --ioengine=libaio --direct=1 --bsrange=512-128k --iodepth=128 --numjobs=1 --size=64m --filename=%s --verify=md5 --verify_fatal=1" % (nbd_device)
...
class TestBlockFioRandW(TestCase.FDSTestCase):
    ...
    def test_BlockFioWrite(self):
        ...
        # TODO(Andrew): Don't hard code all of this stuff...
        fioCmd = "sudo fio --name=rand-writers --readwrite=randwrite --ioengine=libaio --direct=1 --bsrange=512-128k --iodepth=128 --numjobs=1 --size=64m --filename=%s --verify=md5 --verify_fatal=1" % (nbd_device)
        result = subprocess.call(fioCmd, shell=True)
        if result != 0:
            self.log.error("Failed to run write workload")
            return False
