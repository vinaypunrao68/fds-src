...
debug_tool_path = '../../Build/linux-x86_64.debug/tools/fds.debug'
...
class TestRunGarbageCollector(TestCase.FDSTestCase):
    ...
    def test_RunGC(self):
        ...
        pre_gc_dms_refscan = get_dm_service_counters()
        cmd = debug_tool_path + " " + "gc start"
        ...
        while retryCount < maxRetries:
            ...
            if self.gc_not_running() is False:
                ...
                assert verify_counters_timestamp(self.log, pre_gc_dms_refscan) is True
...
def get_dm_service_counters():
    cmd = debug_tool_path + " " + "service counters dm"
    op = local(cmd, capture=True)
...
# If pre_gc_dms_refscan timestamps are greater than resepctive post_gc_dms_refscan then retrun True else False
def verify_counters_timestamp(log, pre_gc_dms_refscan):
    post_gc_run = get_dm_service_counters()
    ...
    if len(pre_gc_dms_refscan) != 0:
        ...
        for idx, value in enumerate(pre_gc_dms_refscan):
            if not int(post_gc_run[idx]) > int(pre_gc_dms_refscan[idx]):
                log.error("Failed: dm.refscan timestamp {0} not greater than previous run {1}".
                          format(post_gc_run[idx], pre_gc_dms_refscan[idx]))
                log.info("dm.refscan timestamps. previous run:{}, Current run {}".
                         format(pre_gc_dms_refscan, post_gc_run))
                return False
    else:
        log.info("This is First GC run so skip dm.refscan.lastrun.timestamp verification")
    return True
