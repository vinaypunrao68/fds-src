...
extern void init_fds_mod(int argc, char **argv);
...
{
    (void)fi;
    printf("read size %ld, off %ld, cuseexmp %ld\n", size, off, cusexmp_size);
    if (off >= cusexmp_size)
        off = cusexmp_size;
...
int main(int argc, char **argv)
{
    ...
    return cuse_lowlevel_main(args.argc, args.argv, &ci, &cusexmp_clop, NULL);
}
