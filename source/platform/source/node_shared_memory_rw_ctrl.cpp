...
namespace fds
{
    ...
    // --------
    //
    int NodeShmRWCtrl::mod_init(SysParams const *const p)
    {
    ...
    // -----------
    //
    void NodeShmRWCtrl::mod_startup()
    {
        void   *shm;
        ...
        shm_node_rw =
            new ShmObjRWKeyUint64(shm_ctrl, shm_node_off, shm_node_hdr->shm_node_inv_key_off,
                                  shm_node_hdr->shm_node_inv_obj_siz,
                                  shm_max_nodes);

        shm_uuid_rw =
            new ShmObjRW(shm_ctrl, shm_uuid_off, shm_node_hdr->shm_uuid_bind_key_siz,
                         shm_node_hdr->shm_uuid_bind_key_off, shm_node_hdr->shm_uuid_bind_obj_siz,
                         shm_max_svc);

        shm_dlt_rw      =
            new ShmObjRW(shm_ctrl, shm_dlt_off, shm_node_hdr->shm_dlt_key_size,
                         shm_node_hdr->shm_dlt_key_off, shm_node_hdr->shm_dlt_size,
                         1);

        shm_dmt_rw      =
            new ShmObjRW(shm_ctrl, shm_dmt_off, shm_node_hdr->shm_dmt_key_size,
                         shm_node_hdr->shm_dmt_key_off, shm_node_hdr->shm_dmt_size,
                         1);

        shm_am_rw   =
            new ShmObjRWKeyUint64(shm_ctrl, shm_am_off, shm_node_hdr->shm_am_inv_key_off,
    ...
    // ------------
    //
    void NodeShmRWCtrl::mod_shutdown()
    {
    ...
    // ---------------
    //
    void NodeShmRWCtrl::shm_init_header(node_shm_inventory_t *hdr)
    {
    ...
    void NodeShmRWCtrl::shm_init_queue(node_shm_queue_t *queue)
    {
    }

    void NodeShmRWCtrl::shm_setup_queue()
    {
        size_t    size;
        void     *area;
        ...
        shm_cons_q = new Shm_nPrd_1Con(&shm_queue->smq_sync, &shm_queue->smq_svc2plat, shm_rw_data);

        /* Platform daemon is the single producer to n number of consumers. */
        shm_prod_q = new Shm_1Prd_nCon(&shm_queue->smq_sync, &shm_queue->smq_plat2svc, shm_rw_data);
    }
}  // namespace fds
