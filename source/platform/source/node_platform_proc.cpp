...
void NodePlatformProc::plf_start_node_services(const fpi::FDSP_ActivateNodeTypePtr &msg) {
    ...
    if (auto_start == true) {
        if (msg->has_sm_service) {
            ...
            args.push_back(util::strformat("--fds.sm.svc.uuid=%lld", plf_node_data.nd_node_uuid));
            pid = fds_spawn_service("StorMgr", proc_root->dir_fdsroot(), args, true);
        ...
        if (msg->has_dm_service) {
            ...
            args.push_back(util::strformat("--fds.dm.svc.uuid=%lld", plf_node_data.nd_node_uuid));

                pid = fds_spawn_service("DataMgr", proc_root->dir_fdsroot(), args, true);
            ...
            if (msg->has_am_service) {
                ...
                pid = fds_spawn_service("AMAgent", proc_root->dir_fdsroot(), args, true);
