...
namespace fds
{
    ...
    PhaseSync::PhaseSync(fpi::SvcUuid &master, NodeAgent::pointer owner,
                         FsmTable::pointer tab) : StateObj(PhaseIdleSt::st_index(),
                                                           tab), ph_master_uuid(master),
        ph_owner(owner)
    {
    }

    void PhaseSync::act_phase_prepare(PhaseEvent::ptr evt)
    {
    }

    void PhaseSync::act_phase_commit(PhaseEvent::ptr evt)
    {
    }

    void PhaseSync::act_phase_abort(PhaseEvent::ptr evt)
    {
    }

    void PhaseSync::act_fmt_prepare_ack(fpi::PhaseSyncPtr &pkt)
    {
    }

    void PhaseSync::act_fmt_commit_ack(fpi::PhaseSyncPtr &pkt)
    {
    }

    void PhaseSync::act_fmt_abort(fpi::PhaseSyncPtr &pkt)
    {
    }

    void PhaseSync::act_fmt_abort_ack(fpi::PhaseSyncPtr &pkt)
    {
    }

    void PhaseSync::act_send_prepare_ack(fpi::SvcUuid *dst, fpi::PhaseSyncPtr &pkt)
    {
    }

    void PhaseSync::act_send_commit_ack(fpi::SvcUuid *dst, fpi::PhaseSyncPtr &pkt)
    {
    }

    void PhaseSync::act_send_abort(fpi::SvcUuid *dst, fpi::PhaseSyncPtr &pkt)
    {
    }

    void PhaseSync::act_send_abort_ack(fpi::SvcUuid *dst, fpi::PhaseSyncPtr &pkt)
    {
    }

    void PhaseSync::act_recv_sync(fpi::AsyncHdrPtr &hdr, fpi::PhaseSyncPtr &pkt)
    {
    }
    ...
    PhaseSyncMaster::PhaseSyncMaster(fpi::SvcUuid &self, NodeAgent::pointer owner,
                                     FsmTable::pointer tab) : PhaseSync(self, owner,
                                                                        tab), ph_commit(NULL),
        ph_sync_ack()
    {
    }

    void PhaseSyncMaster::act_commit_start(AgentArray *arr)
    {
    }

    void PhaseSyncMaster::act_commit_abort()
    {
    }

    void PhaseSyncMaster::act_fmt_phase_prepare(fpi::PhaseSyncPtr &pkt)
    {
    }

    void PhaseSyncMaster::act_fmt_phase_commit(fpi::PhaseSyncPtr &pkt)
    {
    }

    void PhaseSyncMaster::act_fmt_phase_abort(fpi::PhaseSyncPtr &pkt)
    {
    }

    void PhaseSyncMaster::act_send_prepare(fpi::SvcUuid *dst, fpi::PhaseSyncPtr &pkt)
    {
    }

    void PhaseSyncMaster::act_send_commit(fpi::SvcUuid *dst, fpi::PhaseSyncPtr &pkt)
    {
    }
    ...
    PhaseSyncTab::PhaseSyncTab() :    FsmTable(FDS_ARRAY_ELEM(sgt_2phase_tab),
                                               sgt_2phase_tab), ph_master(NULL)
    {
        st_set_switch_board(sgl_2phase_flow, FDS_ARRAY_ELEM(sgl_2phase_flow));
    }

    void PhaseSyncTab::mod_init()
    {
        gl_OmUuid.uuid_assign(&ph_om_dest);
    }

    void PhaseSyncTab::mod_startup()
    {
    }

    void PhaseSyncTab::mod_shutdown()
    {
    }

    void PhaseSyncTab::phase_sync_create(fpi::SvcUuid &peer, NodeAgent::pointer owner)
    {
    }

    void PhaseSyncTab::ph_recv_sync(fpi::AsyncHdrPtr &hdr, fpi::PhaseSyncPtr &pkt)
    {
    }

    void PhaseSyncTab::ph_sync_submit(fpi::AsyncHdrPtr &hdr, PhaseEvent::ptr evt)
    {
    }

    PhaseSync::ptr PhaseSyncTab::ph_sync_alloc(fpi::SvcUuid &svc, NodeAgent::pointer owner)
    {
        return new PhaseSync(svc, owner, this);
    }

    PhaseSyncMaster::ptr PhaseSyncTab::ph_master_alloc(NodeAgent::pointer owner)
    {
        fpi::SvcUuid    om;
