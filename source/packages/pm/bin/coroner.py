...
class FDSCoroner(object):
    ...
    def collect_dir(self, directory, path):
        ...
        if os.path.exists(path):
            ...
            cmd = ["/bin/tar", "-C", path_base, "--exclude-from", self.fdsroot + "/bin/coroner.excludes", "-chzf",
...
def run_collect(opts):
    ...
    bodybag.collect_cmd(command='%s/bin/redis-cli keys *' % bodybag.fdsroot, name='redis-keys')
    ...
    bodybag.collect_cmd(command='/bin/ls -l %s/var/corefiles' % bodybag.fdsroot, name='ls_corefiles')
    if not opts['buildermode']:
        bodybag.collect_cmd(
            command='/sbin/parted --list --script',
            name='parted'
        )
...
def main():
    ...
    parser_collect.add_argument(
            '--fdsroot', dest="fdsroot", default='/opt/fds/formation_one',
