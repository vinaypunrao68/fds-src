...
class FDSBlockVolumeDriver(driver.VolumeDriver):
    ...
    def _attach_fds_block_dev(self, fds, name, ip = None):
        if ip is not None:
            host_spec = 'root@' + ip
            (stdout, stderr) = self._execute('ssh', '-o', 'StrictHostKeyChecking=no', host_spec, 'nbdadm.py', 'attach', self.configuration.fds_nbd_server, name, run_as_root=False)
            return stdout.strip()
        else:
            (stdout, stderr) = self._execute('nbdadm.py', 'attach', self.configuration.fds_nbd_server, name, run_as_root=False)
            return stdout.strip()
    ...
    def _detach_fds_block_dev(self, fds, name, ip = None):
        if ip is not None:
            host_spec = 'root@' + ip
            (stdout, stderr) = self._execute('ssh', '-o', 'StrictHostKeyChecking=no', host_spec, 'nbdadm.py', 'detach', self.configuration.fds_nbd_server, name, run_as_root=False)
        else:
            (stdout, stderr) = self._execute('nbdadm.py', 'detach', self.configuration.fds_nbd_server, name, run_as_root=False)
