...
int StorHvCtrl::fds_move_wr_req_state_machine(const FDSP_MsgHdrTypePtr& rxMsg) {
  ...
  FDS_PLOG_SEV(sh_log, fds::fds_log::normal) << "State transition attemp for trans "
       ...
       if ( journEntry->io->io_type == FDS_IO_READ || journEntry->io->io_type == FDS_GET_BLOB) { 
         ...
         if (endPoint) {
          boost::shared_ptr<FDSP_DataPathReqClient> client =
           ...
           FDS_PLOG(storHvisor->GetLog()) << " StorHvisorRx:" << "IO-XID:" << trans_id << " volID: 0x"
                                          << std::hex << vol_id << std::dec << " - Sent Async getObj req to SM at " << node_ip ;
         ...
         if (endPoint) {
          boost::shared_ptr<FDSP_DataPathReqClient> client =
           ...
           FDS_PLOG(storHvisor->GetLog()) << " StorHvisorRx:" << "IO-XID:" << trans_id << " volID: 0x"
                                          << std::hex << vol_id << std::dec << " - Sent Async deleteObj req to SM at " << node_ip ;
