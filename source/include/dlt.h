...
#ifndef SOURCE_FDS_DLT_H_
#define SOURCE_FDS_DLT_H_
...
namespace fds {
     ...
     */
    struct NodeList {
        NodeList(NodeUuid uid0=0,NodeUuid uid1=0,NodeUuid uid2=0,NodeUuid uid3=0) {
            p=new NodeUuid[MAX_REPL_FACTOR];
            p[0]=uid0;p[1]=uid1;p[2]=uid2;p[3]=uid3;
        }
        
        NodeList(const NodeList& n) {
            p=new NodeUuid[MAX_REPL_FACTOR];
            //TODO!! change to memcpy later
    ...
    public :
        ...
        DLT(uint numTokens=MAX_TOKENS,uint width=MAX_REPL_FACTOR,bool fInit=false);
        DLT(const DLT& dlt);
