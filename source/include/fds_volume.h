...
public:
   ...
   fds_uint32_t   	localDomainId; // Local domain id that this volume belongs to
   fds_uint32_t   	globDomainId;
   fds_volid_t    	volUUID;
   FDS_VolType    	volType; 

   fds_uint64_t   	capacity;
   fds_uint64_t   	maxQuota; // Quota percentage of capacity that should alert 
   fds_uint32_t   	replicaCnt; // Number of replicas reqd for this volume
   fds_uint32_t   	writeQuorum; // Quorum number of writes considered success
   ...
   fds_uint32_t   	placementPolicy; // Ability to change the placement policy
...
public:
  ...
  fds_uint32_t   time2archive; // TTL for unchanged objects to archive to DR site 
...
public:
  ...
  ~FDS_ArchivePolicyTbl();
  Error& AddArchivePolicy(FDS_ArchivePolicy& archive_policy);
  void DeleteArchivePolicy(FDS_ArchivePolicy& archive_policy);
...
public:
  FDS_VolumePolicyTbl();
  ~FDS_VolumePolicyTbl();
  Error& AddVolumePolicy(FDS_VolumePolicy& vol_policy);
  void DeleteVolumePolicy(FDS_VolumePolicy& vol_policy);
