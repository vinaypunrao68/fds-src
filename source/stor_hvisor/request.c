...
int
blktap_request_get_pages(struct fbd_device *tap,
...
static void
blktap_request_put_pages(struct fbd_device *tap,
...
size_t
blktap_request_debug(struct fbd_device *tap, char *buf, size_t size)
{
	struct blktap_page_pool *pool = tap->pool;
	mempool_t *mem = pool->bufs;
...
blktap_request_alloc(struct fbd_device *tap)
{
	struct blktap_request *request;
...
void
blktap_request_free(struct fbd_device *tap,
		    struct blktap_request *request)
{
...
static void
blktap_request_bounce_seg(struct fbd_device *tap,
			  struct blktap_request *request,
			  int seg, int write)
{
...
void
blktap_request_bounce(struct fbd_device *tap,
		      struct blktap_request *request,
		      int write)
{
	int seg;
