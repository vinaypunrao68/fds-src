...
int main(int argc, char** argv)
{
	...
	if (fd == -1) {
		perror("lio_listio_wait: Cannot open testfile");
		return 1;
	}
	...
	for (i = 0; i < NUM_IOS; i++) {
		...
		if (ret != 0) {
			perror ("lio_listio_wait: failed to alloc buffer");
			return 2;
		}
		...
		if (list[i] == NULL) {
			perror ("lio_listio_wait: failed to alloc lio entry");
			return 3;
		}
	...
	/* Run list */
	ret = lio_listio(LIO_WAIT, list, NUM_IOS, NULL);

	if (ret) {
		perror("lio_listio_wait: lio_listio failed");
		return 4;
	}

	if (num_received != NUM_IOS - 2) {
		printf ("lio_listio_wait: lio_listio did not wait\n");
		return 5;
	}
	...
	for (i = 0; i < NUM_IOS - 2; i++) {
		...
		if (err) {
			printf ("lio_listio_wait: req %d error (%s)\n",
				i, strerror (errno));
			return 7;
		}

		if (ret != list[i]->aio_nbytes) {
			printf ("lio_listio_wait: req %d wrong size returned %d/%d\n",
