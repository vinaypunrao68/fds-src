...
void sigrt_handler(int signo, siginfo_t *info, void *context)
{
	...
	if (ret)
		printf("aio_write_one_sig: aio_error error (%s)\n", strerror(ret));
...
int main(int argc, char** argv)
{
	...
	if (buf == NULL) {
		perror("aio_write_one_sig: malloc");
		return 1;
	}
	...
	if (ret) {
		perror("aio_write_one_sig: aio_write error");
		return 4;
	}
	...
	if (len != BLOCK_SIZE) {
		printf ("aio_write_one_sig: aio_return error\n");
		return 6;
	}
