...
static volatile unsigned int num_received = 0;
...
int main(int argc, char** argv)
{
	...
	if (fd == -1) {
		perror("lio_listio_nowait: Cannot open testfile");
		return 1;
	}
	...
	for (i = 0; i < NUM_IOS; i++) {
		...
		if (ret != 0) {
			perror ("lio_listio_nowait: failed to alloc buffer");
			return 2;
		}
		...
		if (list[i] == NULL) {
			perror ("lio_listio_nowait: failed to alloc lio entry");
			return 3;
		}
	...
	/* No notification on list completion */
	...
	if (ret) {
		perror("lio_listio_nowait: lio_listio failed");
		return 4;
	}

	if (num_received == NUM_IOS - 2) {
		printf ("lio_listio_nowait: lio_listio waited\n");
		return 5;
	}

	/* Wait for list processing completion */
	while (num_received != NUM_IOS - 2);

	if (num_received != NUM_IOS - 2) {
		printf ("lio_listio_nowait: wrong number of events received %d/%d\n",
			num_received, NUM_IOS);
		return 6;
	}
	...
	for (i = 0; i < NUM_IOS - 2; i++) {
		...
		if (err) {
			printf ("lio_listio_nowait: req %d error (%s)\n",
				i, strerror (errno));
			return 7;
		}

		if (ret != list[i]->aio_nbytes) {
			printf ("lio_listio_nowait: req %d wrong size returned %d/%d\n",
