...
void notify_cb (void* ptr)
{
	...
	if (ret)
		printf("aio_read_one_thread: aio_error error (%s)\n", strerror(ret));
...
int main(int argc, char** argv)
{
	...
	if (buf == NULL) {
		perror("aio_read_one_thread: malloc");
		return 1;
	}
	...
	if (fd == -1) {
		perror("aio_read_one_thread: Cannot open testfile");
		return 3;
	}
	...
	if (ret) {
		perror("aio_read_one_thread: aio_read error");
		return 4;
	}
	...
	if (len<0) {
		printf ("aio_read_one_thread: aio_return error\n");
		return 6;
	}
