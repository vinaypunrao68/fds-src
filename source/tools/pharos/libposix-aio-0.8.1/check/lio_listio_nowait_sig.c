...
int main(int argc, char** argv)
{
	...
	if (fd == -1) {
		perror("lio_listio_nowait_sig: Cannot open testfile");
		return 1;
	}
	...
	for (i = 0; i < NUM_IOS; i++) {
		...
		if (ret != 0) {
			perror ("lio_listio_nowait_sig: failed to alloc buffer");
			return 2;
		}
		...
		if (list[i] == NULL) {
			perror ("lio_listio_nowait_sig: failed to alloc lio entry");
			return 3;
		}
	...
	if (ret) {
		perror("lio_listio_nowait_sig: lio_listio failed");
		return 4;
	}

	if (received_all) {
		printf ("lio_listio_nowait_sig: lio_listio waited\n");
		return 5;
	}
	...
	if (num_received != NUM_IOS - 2) {
		printf ("lio_listio_nowait_sig: wrong number of events received %d/%d\n",
	...
	for (i = 0; i < NUM_IOS - 2; i++) {
		...
		if (err) {
			printf ("lio_listio_nowait_sig: req %d error (%s)\n",
				i, strerror (errno));
			return 7;
		}

		if (ret != list[i]->aio_nbytes) {
			printf ("lio_listio_nowait_sig: req %d wrong size returned %d/%d\n",
