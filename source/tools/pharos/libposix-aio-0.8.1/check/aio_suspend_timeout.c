...
int main(int argc, char** argv)
{
	...
	if (fd == -1) {
		perror("aio_suspend_timeout: Cannot create testfile");
		return 1;
	}
	...
	for (i = 0; i < NUM_IOS; i++) {
		...
		if (ret != 0) {
			perror ("aio_suspend_timeout: failed to alloc buffer");
			return 2;
		}
		...
		if (aiocbs[i] == NULL) {
			perror ("aio_suspend_timeout: failed to alloc aio entry");
			return 3;
		}
	...
	struct timespec ts = {0, 0}; /* 0 ms */

	ret = lio_listio (LIO_NOWAIT, aiocbs, NUM_IOS, &event);

	if (ret) {
		printf("aio_suspend_timeout: lio_listio failed (%s)\n", strerror(errno));
		return 4;
	}

	if (received_selected) {
		printf ("aio_suspend_timeout: AIOCB 6 already completed before suspend\n");
		return 5;
	}

	ret = aio_suspend((const struct aiocb **)list, 2, &ts);

	if (ret != -1) {
		printf ("aio_suspend_timeout: aio_suspend should have returned -1\n");
		return 6;
	} else if (errno != EAGAIN) {
		printf ("aio_suspend_timeout: aio_suspend should have set errno to EAGAIN : %d, %s\n",
	...
	for (i = 0; i < NUM_IOS; i++) {
		...
		if (err) {
			printf ("aio_suspend_timeout: req %d error (%s)\n",
				i, strerror (errno));
			return 8;
		}

		if (ret != aiocbs[i]->aio_nbytes) {
			printf ("aio_suspend_timeout: req %d wrong size returned %d/%d\n",
