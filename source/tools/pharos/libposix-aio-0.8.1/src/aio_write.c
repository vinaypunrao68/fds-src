...
int __aio_create_write(struct aiocb64 *__aiocbp)
{
	int	ret = 0;
...
#	endif
	...
	__aiocbp->__opcode = IO_CMD_PWRITE;
...
#	ifndef HAVE_BUFFERED_AIO
	...
	if ( ((unsigned long)__aiocbp->aio_buf & 0x1FF) ||
		...
		memcpy (__aiocbp->__buf, (void *)__aiocbp->aio_buf, __aiocbp->aio_nbytes);
	} else {
...
int aio_write64(struct aiocb64 *__aiocbp)
{
	...
	ret = __aio_create_write(__aiocbp);
...
int aio_write(struct aiocb *__aiocbp)
{
	...
	return aio_write64((struct aiocb64 *)__aiocbp);
}
