...
int main(int argc, char *argv[]) {
    ...
    // Get args
    if (argc < 2) {
        std::cout << "Please pass a shared memory segment name.";
        return -1;
    } else {
        name = argv[1];
    }

    if (strstr(name, "-rw-") == nullptr) {
        shm_ctrl = new FdsShmem(name, 2097152);
        ...
        shm_ctrl = new FdsShmem(name, 131072);  // Queue is 128k
        shm = reinterpret_cast<char *>(shm_ctrl->shm_attach(PROT_READ));
