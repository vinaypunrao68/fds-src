...
runtime=-40000        # number of IOs generator will produce ( positive number means length of experiment in seconds) 
blocksize=4          # in KB
...
pushd .; cd $PHAROS_DIR
./Pharos -k $blocksize -o $outstand_count -d w -t $tracename $DEVICE $access $runtime $VOLID $PREFIX
popd
