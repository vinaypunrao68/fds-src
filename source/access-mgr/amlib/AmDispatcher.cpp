...
AmDispatcher::start() {
    ...
    numDmPrimaries = conf.get_abs<fds_uint32_t>("fds.dm.number_of_primary", 2);
}
...
AmDispatcher::createFailoverRequest(fds_volid_t const& volId,
    ...
    int numPrimaries = (dmsForVol->getLength() < numDmPrimaries ? dmsForVol->getLength() : numDmPrimaries);
...
AmDispatcher::dispatchSetVolumeMetadata(AmRequest *amReq) {
    ...
    volMetaMsg->sequence_id = volReq->vol_sequence;
