...
run_deploy_playbook() {
    ...
    cd ${ansible_base_dir} && ansible-playbook ${ansible_args} -e "deploy_artifact=${deploy_source}" -e "fds_cluster_name=${inventory_filename}" --tags fdsconfig,local_check,shutdown,startup --vault-password-file ~/.vault_pass.txt
}
