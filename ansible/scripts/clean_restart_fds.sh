...
cat << EOF
Usage:
${0##/*} inventory-file [debug]
Clean FDS on a group of hosts defined in an Ansible inventory file.
...
run_deploy_playbook() {
    ...
    cd ${ansible_base_dir} && ansible-playbook ${ansible_args} -e "deploy_artifact=${deploy_source}" -e "fds_cluster_name=${inventory_filename}" -e "clean_fds=yes" -e "force=true" --skip-tags uninstall,setup,install-deps,install-local,install-repo,post-install --vault-password-file ~/.vault_pass.txt
}
