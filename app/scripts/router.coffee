define (require, exports, modules) ->
  ...
  VolumesIndex  = require('views/volumes/index')
  VolumesShow   = require('views/volumes/show')
  ...
  class Router extends Backbone.Router
    ...
    volumes: ->
      @volumes_view = new VolumesIndex(volumes: @volumes)
    ...
    volume: (volume_id)->
      @fetchVolumes =>
        ...
        volume_view = new VolumesShow(model: @volume)
    ...
    volumeEdit: (volume_id) ->
      @fetchVolumes =>
        @volume = @volumes.findWhere({id: +volume_id})
        volume_view = new VolumesForm(model: @volume)
