angular.module( 'debug' ).controller( 'debugController', ['$scope', '$http', '$base64', '$interval', '$stats_push_helper', '$byte_converter', function( $scope, $http, $base64, $interval, $stats_push_helper, $byte_converter ){
    ...
    $scope.byteLabel = function( value ){
        
        return $byte_converter.convertBytesToString( value, 1 );
    };
    ...
    $scope.statChoices = [{name: 'None'},
                          ...
                          {name: 'Logical bytes', value: 'LBYTES' },
                          {name: 'Physical Bytes', value: 'PBYTES'}];
    ...
    $scope.chartLabel = function( value ){
        
        // this = Scope of the chart
        var metric = this.data.series[0].type;
        for ( var i = 0; i < $scope.chartChoice.length; i++ ){
            if ( $scope.chartChoice[i].value === metric ){
                if ( angular.isFunction( $scope.chartChoice[i].label ) ){
                    return $scope.chartChoice[i].label( value );
                }
            }
        }
    ...
    $scope.start = function(){
        ...
        // make sure each chart is set to the right kind of data
        for ( var ic = 0; ic < $scope.chartData.length; ic++ ){
            $scope.chartData[ic].series[0].type = $scope.chartChoice[ic].value;
        }
    ...
    var handleStats = function( response ){
        ...
        if ( response.data && response.data.length > 0 ){
            console.log( 'Data received.  ' + response.data.length + ' points' );
        }
    ...
    $scope.$watchCollection( 'chartChoice', function( newVal, oldVal ){
        console.log( 'Changed' );
    });
