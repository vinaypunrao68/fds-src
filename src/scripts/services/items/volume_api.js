...
//        }];
    ...
    api.getQosPolicyPresets = function( callback, failure ){
        return $http_fds.get( webPrefix + '/presets/quality_of_service_policies', function( presets ){
            // the server may return bad values for the iopsMin where 20 is really the lowest possible.
            for ( var i = 0; angular.isDefined( presets ) && i < presets.length; i++ ){
                
                var preset = presets[i];
                
                if ( preset.iopsMin < api.MIN_GUARANTEE_VALUE ){
                    preset.iopsMin = api.MIN_GUARANTEE_VALUE;
                }
            }
