...
//        }
    ...
    $scope.save = function(){
        ...
        if ( angular.isDefined( $scope.volumeVars.toClone ) && $scope.volumeVars.toClone.value === 'clone' ){
            ...
            if ( volume.timelineTime === $scope.volumeVars.now ){
                // this makes sure the system does it from "now" in case the actual
                // time hits some sort of boundary condition.
                volume.timelineTime = 0;
            }
    ...
    $scope.$watch( 'volumeVars.creating', function( newVal ){
        if ( newVal === true ){
            ...
            $scope.newQos = {
                iopsMin: $volume_api.MIN_GUARANTEE_VALUE,
                iopsMax: 0,
                priority: 7
            };
